...
export default function ContactsPage() {
  ...
  const loadContacts = async (page = 1, search = "", companyFilter = "", importanceFilter = null, sort = sortBy, order = sortOrder, hasBusinessCard = null, retryCount = 0) => {
    ...
    try {
      ...
      if (search && result.data.length === 0) {
        showToast('info', '検索結果なし', `「${search}」に一致する連絡先が見つかりませんでした`);
      }
    } catch (error) {
      ...
      if (retryCount < 2) {
        setTimeout(() => {
          loadContacts(page, search, companyFilter, importanceFilter, sort, order, hasBusinessCard, retryCount + 1);
        }, 1000 * (retryCount + 1));
        showToast('warning', '読み込み中...', 'データを再取得しています');
      } else {
        showToast('error', 'データ取得エラー', 'データの取得に失敗しました。ページを更新してください');
        // エラー時でも空配列をセット
        setData([]);
        setFilteredData([]);
        setTotalCount(0);
  ...
  // 連絡先選択の管理  const handleSelectContact = (contactId: string, checked: boolean) => {
    if (checked) {
  ...
  const sendBulkEmail = async () => {
    ...
    if (!bulkEmailForm.subject || !bulkEmailForm.content) {
      showToast('warning', '入力エラー', '件名と本文を入力してください');
      return;
    }
  ...
  // 複数OCR処理：複数画像を同時に自動実行版
  const processMultipleOcrAuto = async (files: File[]) => {
    // 即座にモーダルを閉じて処理開始
    setShowOcrModal(false);
    setOcrLoading(true);
    showToast('info', '一括スキャン開始', `${files.length}枚の名刺をOCR処理しています。完了したら自動で一覧に追加されます。`);
    ...
    try {
      ...
      const promises = files.map(async (file) => {
        try {
          ...
          if (response.ok && result.contact) {
            successCount++;
            processedFiles.push(result.contact.fullName || file.name);
            
            // OCR品質情報をログ出力
            const qualityInfo = result.ocrQuality ? 
              `(品質: ${result.ocrQuality.level}, 信頼度: ${result.ocrQuality.confidence?.toFixed(2) || 'N/A'})` : 
              '';
            
            return { success: true, contact: result.contact, fileName: file.name, quality: result.ocrQuality };
          } else {
            failCount++;
          ...
          failCount++;
          return { success: false, error: error instanceof Error ? error.message : String(error), fileName: file.name };
        }
      });
      ...
      if (successCount > 0 && failCount === 0) {
        showToast('success', '✅ 一括OCR完了', `${successCount}枚すべて正常に登録されました`);
      } else if (successCount > 0 && failCount > 0) {
        showToast('warning', '⚠️ 一括OCR完了（一部失敗）', `成功: ${successCount}枚、失敗: ${failCount}枚`);
      } else {
        showToast('error', '❌ 一括OCR失敗', `${failCount}枚すべて失敗しました`);
      }
      
      // リストを自動更新（バックグラウンドで実行）
      if (successCount > 0) {
      ...
      showToast('error', '❌一括OCRエラー', error instanceof Error ? error.message : String(error));
    } finally {
      setOcrLoading(false);
    }
  };
  ...
  // OCR処理：画像選択と同時に自動実行版
  const processOcrAuto = async (file: File) => {
    // 複数処理関数を使用して単一ファイルを処理
    return processMultipleOcrAuto([file]);
  };

  // 旧来の単一OCR処理関数（互換性のため残す）
  const processOcrAutoSingle = async (file: File) => {
    // 即座にモーダルを閉じて処理開始
    setShowOcrModal(false);
    setOcrLoading(true);
    ...
    try {
      ...
      if (response.ok) {
        if (result.contact) {
          ...
          // リストを自動更新（バックグラウンドで）          loadContacts(currentPage, searchTerm, filters.company, filters.hasPhone ? 1 : null, sortBy, sortOrder, filters.hasBusinessCard ? 1 : null);
  ...
  const handleSearch = (searchValue = searchTerm) => {
    ...
    // 検索中インジケータを表示
    if (searchValue) {
      showToast('info', '検索中...', `「${searchValue}」を検索しています`, 2000);
    }
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (searchTerm !== "" || Object.values(filters).some(v => v !== "" && v !== false)) {
        ...
        // 検索条件が空の場合、全件表示
        loadContacts(1, "", "", null, sortBy, sortOrder, null);
      }
    }, 300); // デバウンス: 300ms後に検索実行（高速化）
  ...
  // 画像アップロード
  const uploadImage = async (file: File, type: "businessCard" | "profile", isEdit: boolean = false) => {
    try {
      const formData = new FormData();
      formData.append('file', file);
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${
      ...
      {/* アクションバー */}
      <div className={`sticky top-16 z-30 border-b backdrop-blur-lg transition-all duration-300 ${
        isDarkMode 
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <button
                ...
                className="px-3 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:from-blue-600 hover:to-indigo-700 transition-all duration-200 text-sm font-medium shadow-md hover:shadow-lg transform hover:scale-105"
              >
                 名刺OCR
              </button>
              <button
              ...
              <button
                ...
                className="px-3 py-2 bg-gradient-to-r from-blue-500 to-green-600 text-white rounded-lg hover:from-blue-600 hover:to-green-700 transition-all duration-200 text-sm font-medium shadow-md hover:shadow-lg transform hover:scale-105"
              >
                 CSVエクスポート              </button>
            </div>
          </div>
            ...
            <div className="p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row gap-4 items-center">
                <div className="flex-1 relative w-full">
                  ...
                  <input
                    type="text"
                    placeholder="会社名・氏名・Email・電話番号を検索"
                    value={searchTerm}
                    onChange={(e) => {
                    ...
                    className={`w-full pl-12 pr-${searchTerm ? '10' : '4'} py-3 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                      isDarkMode 
                        ? 'bg-slate-700 border-slate-600 text-slate-100 placeholder-gray-400' 
                        : 'bg-slate-50 border-slate-300 text-slate-900 placeholder-gray-500'
                    }`}
                  />
                  {/* 検索中インジケータ */}
                  {isSearching && (
                  ...
                  {searchTerm && !isSearching && (
                    <button
                      ...
                      className="absolute right-12 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                      title="検索をクリア"
                    >
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  {[
                    ...
                    { key: 'company', label: '会社', placeholder: '会社名で絞り込み', icon: '' },
      ...
      <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                      ...
                      <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            className="mr-2 rounded border-slate-300 text-blue-600 focus:ring-blue-500 focus:ring-offset-2"
                  ...
                  <tbody className={`divide-y ${
                    isDarkMode ? 'divide-slate-700 bg-slate-800' : 'divide-slate-200 bg-white'
                  }`}>
              ...
              <div className={`p-4 rounded-lg ${
                ...
                <label className="flex items-center">
                  ...
                  <span className={`text-sm font-medium ${
                    ...
                    全選抁E({selectedContacts.length}件選択中)
                  </span>
                </label>
              </div>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center flex-1 min-w-0">
                        <input
                          ...
                          checked={selectedContacts.includes(contact.id)}
                          onChange={(e) => handleSelectContact(contact.id, e.target.checked)}
                          className="mr-3 rounded border-slate-300 text-blue-600 focus:ring-blue-500 flex-shrink-0"
                        />
                        ...
                        <div className="flex-1 min-w-0">
                          <Link 
                            href={`/contacts/${contact.id}`}
                            className={`text-lg font-semibold block truncate hover:text-blue-500 transition-colors cursor-pointer ${
                              isDarkMode ? 'text-slate-100' : 'text-slate-900'
                            }`}
                          >
                            {contact.fullName}
                  ...
                  {importResult && (
                    <div className={`p-3 rounded-lg ${
                      ...
                      <p className={`text-sm ${
                        ...
                        失敁E <span className="text-red-600 font-medium">{importResult.errorCount}</span>件
                      </p>
              ...
              <div className="p-6">
                ...
                <div className={`mb-6 p-4 rounded-lg ${
                  ...
                  <ol className={`list-decimal list-inside text-sm space-y-1 ${
                    ...
                    <li>複数の名刺画像を選択すると自動で一括スキャン開始</li>
                    <li>選択と同時に画面遷移してバックグラウンドで並列処理</li>
                    <li>名刺から氏名、会社名、電話番号、メールアドレスを自動抽出</li>
                    <li>処理完了後に連絡先リストに一括追加</li>
                  </ol>
                </div>
                ...
                <div className="mb-6">
                  <label className={`text-sm font-medium block mb-2 ${
                    isDarkMode ? 'text-slate-300' : 'text-slate-700'
                     ...
                     名刺画像を選択（複数選択可・選択と同時にスキャン開始！）                  </label>
                  <input
                    type="file"
                    accept="image/*"
                    multiple
